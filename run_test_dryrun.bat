...
set LOG_FILE=%LOG_DIR%\test_dryrun_%TODAY%.log

echo [%DATE% %TIME%] TEST_DRYRUN START >> "%LOG_FILE%"
%PYTHON_EXE% post_kdp_from_csv.py --next-round-robin --dry-run >> "%LOG_FILE%" 2>&1
echo [%DATE% %TIME%] TEST_DRYRUN END (exit=%ERRORLEVEL%) >> "%LOG_FILE%"
