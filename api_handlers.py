...
- ターゲットユーザーが「壁紙にしたくなる」「保存して眺めたくなる」ような高い美的センスを追求すること。"""
    ...
    if not result:
        return {"prompts": [topic], "labels": [topic]}

    numbered = "\n".join([f"{i+1}. {p[:200]}" for i, p in enumerate(result)])
    label_prompt = f"次の英語の画像生成プロンプトに、それぞれ日本語で30文字以内の短いタイトルをつけてください。番号付きリストのみ出力。\n{numbered}"
    labels = [f"パターン{i+1}" for i in range(len(result))]
    try:
        r = requests.post(
            f"https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key={GEMINI_API_KEY}",
            json={"contents": [{"parts": [{"text": label_prompt}]}]}, timeout=15
        )
        if r.status_code == 200:
            lines = r.json()['candidates'][0]['content']['parts'][0]['text'].strip().split('\n')
            parsed = [l.split('. ', 1)[-1].strip() for l in lines if l.strip() and l.strip()[0].isdigit()]
            if len(parsed) == len(result):
                labels = parsed
    except Exception as e:
        print(f"[DEBUG] Label translate error: {e}")

    return {"prompts": result, "labels": labels}
